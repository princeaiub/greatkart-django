$(document).ready(function() {
    $('#add_category_from').on('submit', function(e) {        
        e.preventDefault();
        var formData = new FormData(this);
        $('#loader').show();        
        
        $.ajax({
            url: add_category,  // assuming you set data-url="{% url 'add_user' %}" in your form
            type: "POST",
            data: formData,
            ...
            success: function(response) {
                ...
                // Clear old messages
                $('#formMessages_add_category').stop(true, true).hide().empty();
                ...
                if (response.success) {
                    ...
                    $('#add_category_from')[0].reset();
                    $('#formMessages_add_category').html(messageHtml).fadeIn(200);
                    // setTimeout(function(){
                    //     location.reload();
                    ...
                    // ❌ validation errors
                    if (response.cat_errors) {
                        messageHtml = "<div class='alert alert-danger' role='alert'><ul>";
                        response.cat_errors.forEach(function(err) {
                ...
                setTimeout(function() {
                    $('#formMessages_add_category').fadeOut("slow");
                }, 3000);
            },
            error: function(xhr, status, error) {
                $('#loader').hide();
                $('#formMessages_add_category').stop(true, true).hide().html(
